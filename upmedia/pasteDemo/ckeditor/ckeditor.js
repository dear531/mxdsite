...
function upload(editor) {
    ...
    var serverImgPath = "http://10.0.6.57/uploadImg/";
    ...
    if (null != arrData) {
        ...
        for (var i = 0; i < arrData.length; i++) {
            ...
            alert(clientImgPath);
            var newImageName = i.toString() + clientImgPath.substr(clientImgPath.lastIndexOf("."));

            //查找插件，利用插件进行图片上传
            var a = document.getElementById("objUploadImgActivex");

            //接受插件post的数据，newfilename是新文件存放名称,clientImgPath是本地上传的图片路径(c:/test.jpg)
            var result = a.UpLoadToServer("http://10.0.6.57/imgUpload.php?newfilename=" + newImageName, clientImgPath);
